...
envData[,day_of_year:=as.Date(date,format="%j")]
...
fillRiver<-function(rivers){
  if(sum(is.na(rivers))==0){return(rivers)} #if there aren't any NAs just return the input

  if(length(unique(rivers[!is.na(rivers)]))==1){#if it was only observed in one river, fill with that river
    rivers[1:length(rivers)]<-rivers[!is.na(rivers)][1]}

  if(is.na(rivers[1])){ #if the first one is NA, then fill back from the first actual observation
    rivers[1:which(!is.na(rivers))[1]]<-rivers[which(!is.na(rivers))[1]]
  }
  
  for(i in which(is.na(rivers))){# fill in the rest of the NAs by looking back to the last non-NA
    rivers[i]<-rivers[max(which(!is.na(rivers[1:i])))]
  }
  
  return(rivers)
}

#faster way to fill samples prior to first observation than the fillRiver function
dMData[,riverN:=ifelse(sampleNum<first,riverN[which(!is.na(riverN))[1]],riverN),by=tag]

#then fill the rest of the NAs
dMData[,riverN:=fillRiver(riverN),by=tag]
